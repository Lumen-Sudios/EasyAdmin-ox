        ...
        if not mainMenu or not mainMenu:Visible() then
            ...
            if strings and isAdmin then
                ...
                if permissions["player.ban.permanent"] then
                    table.insert(banLength, {
                        label = GetLocalisedText("permanent"),
                        time = 10444633200
                    })
                end
                ...
                if permissions["player.ban.temporary"] then
                    table.insert(banLength, {
                        label = "6 " .. GetLocalisedText("hours"),
                        time = 21600
                    })
                    table.insert(banLength, {
                        label = "12 " .. GetLocalisedText("hours"),
                        time = 43200
                    })
                    table.insert(banLength, {
                        label = "1 " .. GetLocalisedText("day"),
                        time = 86400
                    })
                    table.insert(banLength, {
                        label = "3 " .. GetLocalisedText("days"),
                        time = 259200
                    })
                    table.insert(banLength, {
                        label = "1 " .. GetLocalisedText("week"),
                        time = 518400
                    })
                    table.insert(banLength, {
                        label = "2 " .. GetLocalisedText("weeks"),
                        time = 1123200
                    })
                    table.insert(banLength, {
                        label = "1 " .. GetLocalisedText("month"),
                        time = 2678400
                    })
                    table.insert(banLength, {
                        label = "1 " .. GetLocalisedText("year"),
                        time = 31536000
                    })
                    table.insert(banLength, {
                        label = GetLocalisedText("customtime"),
                        time = -1
                    })
                end
                GenerateMenu()
...
function GenerateMenu()
    ...
    if DoesPlayerHavePermissionForCategory(-1, "player") then
        ...
        for i, thePlayer in pairs(players) do
            ...
            playerMenus[tostring(thePlayer.id)] = {
                menu = thisPlayerMenu,
                name = thePlayer.name,
                id = thePlayer.id
            }
                ...
                if not playerMenus[tostring(thePlayer.id)].generated then
                    ...
                    if permissions["player.kick"] then
                        ...
                        thisItem.Activated = function(ParentMenu, SelectedItem)
                            local input = lib.inputDialog('Kick Player', {{
                                ...
                                description = 'Provide a reason for kicking the player.',
                                required = true,
                                min = 3,
                                max = 128
                            }})
                    ...
                    if permissions["player.mute"] then
                        local thisItem = NativeUI.CreateCheckboxItem(GetLocalisedText("mute"),
                            MutedPlayers[thePlayer.id], GetLocalisedText("muteguide"))
                        thisPlayer:AddItem(thisItem)
                    ...
                    if permissions["player.teleport.single"] then
                        local sl = {GetLocalisedText("teleporttoplayer"), GetLocalisedText("teleportplayertome"),
                                    GetLocalisedText("teleportmeback"), GetLocalisedText("teleportplayerback"),
                                    GetLocalisedText("teleportintoclosestvehicle")}
                    ...
                    if permissions["player.slap"] then
                        local thisItem = NativeUI.CreateSliderItem(GetLocalisedText("slapplayer"), SlapAmount, 20,
                            false, false)
                        thisPlayer:AddItem(thisItem)
                    ...
                    if permissions["player.warn"] then
                        ...
                        thisItem.Activated = function(ParentMenu, SelectedItem)
                            local input = lib.inputDialog('Warn Player', {{
                                ...
                                description = 'Provide a reason for warning the player.',
                                required = true,
                                min = 3,
                                max = 128
                            }})
        ...
        if permissions["player.reports.view"] then
            ...
            for i, report in pairs(reports) do
                ...
                local thisMenu = _menuPool:AddSubMenu(reportViewer, reportColour .. "#" .. report.id .. " " ..
                    string.sub((report.reportedName or report.reporterName), 1, 12) .. "~w~", "", true)
        ...
        if permissions["player.teleport.everyone"] then
            ...
            local thisItem =
                NativeUI.CreateItem(GetLocalisedText("teleporttome"), GetLocalisedText("teleporttomeguide"))
            ...
            thisItem.Activated = function(ParentMenu, SelectedItem)
                ...
                lib.notify({
                    title = "EasyAdmin",
                    description = "Teleporting All Players to you!",
                    type = "info"
                })
            end
        end
                ...
                if permissions["player.ban.temporary"] or permissions["player.ban.permanent"] then
                    for i, cachedplayer in pairs(cachedplayers) do
                        if cachedplayer.droppedTime and not cachedplayer.immune then
                            ...
                            thisCachedPlayerMenu.ParentItem.Activated =
                                function(ParentMenu, SelectedItem)
                                    thisPlayer = thisCachedPlayerMenu
                                    if not cachedMenus[tostring(cachedplayer.id)].generated then
                                        local thisBanMenu =
                                            _menuPool:AddSubMenu(thisPlayer, GetLocalisedText("banplayer"), "", true)
                                        ...
                                        thisItem.Activated =
                                            function(ParentMenu, SelectedItem)
                                                ...
                                                TriggerServerEvent("EasyAdmin:offlinebanPlayer", cachedplayer.id,
                                                    BanReason, banLength[BanTime].time, cachedplayer.name)
                                                BanTime = 1
                                                BanReason = ""
                                                _menuPool:CloseAllMenus()
                                                Citizen.Wait(800)
    ...
    if DoesPlayerHavePermissionForCategory(-1, "server") then
        if permissions["server.announce"] then
            local thisItem =
                NativeUI.CreateItem(GetLocalisedText("announcement"), GetLocalisedText("announcementguide"))
        ...
        if permissions["server.convars"] then
            ...
            thisItem.Activated = function(ParentMenu, SelectedItem)
                local input = lib.inputDialog('Set Game Type', {{
                    type = 'input',
                    label = 'Game Type',
                    description = 'Enter the new game type',
                    required = true
                }})
            ...
            thisItem.Activated = function(ParentMenu, SelectedItem)
                local input = lib.inputDialog('Set Map Name', {{
                    type = 'input',
                    label = 'Map Name',
                    description = 'Enter the new map name',
                    required = true
                }})
        ...
        if permissions["server.resources.start"] then
            ...
            thisItem.Activated = function(ParentMenu, SelectedItem)
                local input = lib.inputDialog('Start Resource', {{
                    type = 'input',
                    label = 'Resource Name',
                    description = 'Enter the name of the resource to start',
                    required = true
                }})
        ...
        if permissions["server.resources.stop"] then
            ...
            thisItem.Activated = function(ParentMenu, SelectedItem)
                local input = lib.inputDialog('Stop Resource', {{
                    type = 'input',
                    label = 'Resource Name',
                    description = 'Enter the name of the resource to stop',
                    required = true
                }})
        ...
        if permissions["server.convars"] then
            ...
            thisItem.Activated = function(ParentMenu, SelectedItem)
                local input = lib.inputDialog('Set Convar', {{
                    type = 'input',
                    label = 'Convar Name',
                    description = 'Enter the name of the convar',
                    required = true
                }, {
                    type = 'input',
                    label = 'Convar Value',
                    description = 'Enter the value for the convar',
                    required = true
                }})
    ...
    if permissions["player.ban.view"] then
        ...
        local function generateBanOverview(banId)
            ...
            if permissions["player.ban.edit"] then
                local thisItem = NativeUI.CreateItem(GetLocalisedText("savebanchanges"),
                    GetLocalisedText("savebanguide"))
                mainMenu:AddItem(thisItem)
                thisItem.Activated = function(ParentMenu, SelectedItem)
    ...
    if DoesPlayerHavePermissionForCategory(-1, "server") then
        ...
        if #sl > 0 and not RedM then
            ...
            thisItem.Activated = function(ParentMenu, SelectedItem)
                local input = lib.inputDialog('Clean Area', {{
                    ...
                    options = {{
                        value = 'cars',
                        label = GetLocalisedText('cars')
                    }, {
                        value = 'peds',
                        label = GetLocalisedText('peds')
                    }, {
                        value = 'props',
                        label = GetLocalisedText('props')
                    }},
                    default = 'cars'
                }, {
                    type = 'select',
        ...
        if permissions["server.permissions.read"] then
            ...
            for i, ace in pairs(add_aces) do
                ...
                if (ace.file) then
                    local thisItem =
                        NativeUI.CreateItem(GetLocalisedText("location"), GetLocalisedText("locationguide"))
        ...
        thisItem.OnListSelected = function(sender, item, index)
            if item == thisItem then
                ...
                if i == GetLocalisedText("unbanreasons") then
                    ...
                    lib.notify({
                        title = "EasyAdmin",
                        description = "Showing unban Reasons",
                        type = "info"
                    })
                else
                    showLicenses = true
                    lib.notify({
                        title = "EasyAdmin",
                        description = "Showing unban Licenses",
                        type = "info"
                    })
                end
            end
    ...
    if permissions["player.ban.view"] then
        ...
        thisItem.Activated = function(ParentMenu, SelectedItem)
            ...
            lib.notify({
                title = "EasyAdmin",
                description = "Banlist Refreshed Please Reopen EasyAdmin",
                type = "info"
            })
            repeat
                Wait(500)
    ...
    if permissions["player.ban.temporary"] or permissions["player.ban.permanent"] then
        ...
        thisItem.Activated = function(ParentMenu, SelectedItem)
            ...
            lib.notify({
                title = "EasyAdmin",
                description = "Cached Players Refreshed Please Reopen EasyAdmin",
                type = "info"
            })
            repeat
                Wait(500)
    ...
    thisItem.Activated = function(ParentMenu, SelectedItem)
        ...
        lib.notify({
            title = "EasyAdmin",
            description = "Permissions Refreshed Please Reopen EasyAdmin ",
            type = "info"
        })
    end
    ...
    if permissions["anon"] then
        ...
        thisItem.CheckboxEvent = function(sender, item, checked_)
            ...
            -- Notify whether the setting was enabled or disabled
            local status = checked_ and "enabled" or "disabled"
            lib.notify({
                title = "EasyAdmin",
                description = "Anonymous mode has been " .. status .. ".",
                type = "info"
            })
        end
    end
    ...
    thisItem.CheckboxEvent = function(sender, item, checked_)
        ...
        lib.notify({
            title = "EasyAdmin",
            description = "Screen Reader (TTS) mode has been " .. status .. ".",
            type = "info"
        })
    ...
    local thisItem = NativeUI.CreateListItem(GetLocalisedText("screenreaderspeed"), speeds, orientationIndex,
        GetLocalisedText("screenreaderspeedguide"))
    settingsMenu:AddItem(thisItem)
    thisItem.OnListSelected = function(sender, item, index)
